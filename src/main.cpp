...
// Copyright (c) 2017-2020 The Zotova developers
...
#if defined(NDEBUG)
#error "zotova cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uzoa) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight <= 1000001 && nHeight > 1) {
        nSubsidy = 5 * COIN;
    } else if (nHeight <= 1100001 && nHeight > 1000001) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <= 1150001 && nHeight > 1100001) {
        nSubsidy = 1 * COIN;
    } else if (nHeight <= 1200001 && nHeight > 1200001) {
        nSubsidy = 0.5 * COIN;
    } else if (nHeight <= 1210001 && nHeight > 1210001) {
        nSubsidy = 0.25 * COIN;
    } else if (nHeight <= 1510001 && nHeight > 1510001) {
        nSubsidy = 0.05 * COIN;
    } else if (nHeight <= 1610001 && nHeight > 1610001) {
        nSubsidy = 0.025 * COIN;
    } else {
        nSubsidy = 0,02 * COIN;
    }
...
{
    int64_t ret = (nHeight > 500 ? blockValue  / 100 * 80 : 0);
    return ret;
}
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired)
        return error("%s: incorrect proof of work at %d %e %f %g", __func__, pindexPrev->nHeight + 1, block.nBits, nBitsRequired);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ZOA: We use certain sporks during IBD, so check to see if they are
