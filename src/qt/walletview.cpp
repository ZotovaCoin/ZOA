...
// Copyright (c) 2019 The Zotova developers
                                          ...
                                          walletModel(0)
{
    ...
    QFrame *frame_BGHistory = new QFrame(transactionsPage);
    frame_BGHistory->setObjectName(QStringLiteral("frame_BGHistory"));
    frame_BGHistory->setMinimumSize(QSize(960, 682));
    frame_BGHistory->setMaximumSize(QSize(16777215, 16777215));
    frame_BGHistory->setFrameShape(QFrame::StyledPanel);
    frame_BGHistory->setFrameShadow(QFrame::Raised);


    // Create Header with the same names as the other forms to be CSS-Id compatible
    QFrame *frame_Header = new QFrame(frame_BGHistory);
    ...
    QVBoxLayout* vbox = new QVBoxLayout(frame_Header);
